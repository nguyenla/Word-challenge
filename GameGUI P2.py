...
#buttons
        ...
        self.start=ttk.Button(master,text="Start",width=5,command=self.update_clock(120))
        self.start.pack()
        self.start.place(x=150,y=550,anchor=CENTER)

#labels for letter
        
        self.genletters=self.labels(master,20,6,400,60,60,65) #array of random  letter
        self.clickedletters=elf.labels(master,20,6,300,60,60,65) #array of cliked letters
...
#labels for words
        ...
        for i in range (0,10):
            for j in range(0,3):
                self.words3[i][j].configure(text=chr(65+j))
    ...
    def update_clock(self, remaining):
        ...
        if time_remaining <= 0 :
           self.timer.configure(text = "Time's Up")
            ...
            self.timer.configure(text = "%d" % time_remaining)
            time_remaining -= 1
            self.master.after(1000, self.update_clock(time_remaining))
